...
Controller.prototype.sendView = function(context, view, mime) {
	...
	FS.readFile(viewinfo.file, function (err, data) {
		if (err) {
			...
			if (1 || mime === 'text/html') {
				context.view.once('render', function(text) {
...
function Widget(modulePath, nodeModule) {
	Controller.call(this, modulePath, nodeModule)
}
Util.inherits(Widget, Controller)

Widget.prototype._widgetpath = function() {
	return Path.resolve(Config.widget.viewPath, this.id)
}

Widget.prototype.sendView = function(context, view, mime) {
	var self = this
	view = view || context.action
	var viewpath = Path.resolve(this._widgetpath(), view)
	var viewinfo = this._findview(viewpath)
	if (!viewinfo) {
		console.log('Widget view for %s not found'.format(viewpath))
		context.io.text('Widget not found')
		return
	}
	...
	FS.readFile(viewinfo.file, function (err, data) {
		if (err) {
			context.io.text('Widget not found') // TODO - error pages
		} else {
			var view = new View(context.view.template)
			context.view.once('render', function(text) {
				context.io.text(text)
			}).render(data, null, template_options)
		}
	})
