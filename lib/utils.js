...
/* Fields of an object thats not part of the class */
...
/* Fields of an object that comes from the leaf class */
Launch.classFields = function(self, callback, context) {
	var grandParent = self.constructor.super_
	for (var i in self) {
		if (self.constructor.prototype[i] !== undefined &&
		   grandParent.prototype[i] === undefined)
			context = callback(i, self[i], context)
	}	
	return context
}
...
root.MakeWidget = function(filename, mod) {
	var MyWidget = function() {
		Widget.call(this, filename, mod)
	}
	Util.inherits(MyWidget, Widget)
	return MyWidget
}
