...
View.prototype.render = function(data, template_data, template_options) {
	...
	if (this.npending > 0) {
		...
		template.applyTemplate(data, this.viewVars, null, template_options).once('ready', function(view) {
			...
			if (template_data) {
				...
				template.applyTemplate(template_data, templateVars, null, template_options).once('ready', emitView)
			} else {
				emitView(view)
			}
		})
